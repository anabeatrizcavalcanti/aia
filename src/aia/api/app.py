"""API local para usar o AIA no navegador."""
...
from aia.generation.rag_generator import RagGenerator  # noqa: E402
from aia.observability import traceable  # noqa: E402
from aia.retrieval.paths import corpus_dir  # noqa: E402
...
class ChatHistoryMessage(BaseModel):
    """Mensagem recente do chat usada apenas para interpretar continuações."""
...
app = FastAPI(title="AIA Web", version="0.1.0")
...
def favicon():
    """Serve o favicon SVG usado pela interface web."""
    favicon_path = WEB_PUBLIC_DIR / "favicon.svg"
...
def health() -> dict[str, str]:
    ...
    return {"status": "ok", "service": "aia-web"}
...
def chat(request: ChatRequest) -> JSONResponse:
    ...
    try:
        answer = get_generator().answer(
            question,
            filters=filters,
            chat_history=history_to_generator_payload(request.history),
        )
...
def history_to_generator_payload(history: list[ChatHistoryMessage], max_messages: int = 8) -> list[dict[str, str]]:
    """Converte histórico da API para o formato mínimo aceito pelo gerador."""
    payload: list[dict[str, str]] = []
    for item in history[-max_messages:]:
        role = item.role.strip().lower()
        if role not in {"user", "assistant", "bot"}:
            continue
        text = (item.question or item.answer or item.content or "").strip()
        if not text:
            continue
        payload.append({"role": role, "content": " ".join(text.split())[:1600]})
    return payload
...
def answer_to_response(answer: RagAnswer) -> dict[str, Any]:
    ...
    retrieval_metadata = (
        retrieval_package.get("metadata")
        if isinstance(retrieval_package.get("metadata"), dict)
        else {}
    )
    ...
    compact_metadata = {
        ...
        "retrieval_candidates": retrieval_metadata.get("retrieval_candidates", []),
